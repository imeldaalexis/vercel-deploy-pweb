...
app.get('/file/:id', (req, res) => {
  ...
  // Find the file metadata in the fs.files collection
  db.collection('fs.files').findOne({ _id: new mongoose.Types.ObjectId(fileId) }, (err, file) => {
    if (err || !file) {
    ...
    // Find the file chunks in the fs.chunks collection and stream them
    const chunks = db.collection('fs.chunks').find({ files_id: new mongoose.Types.ObjectId(fileId) });
    
    chunks.forEach(chunk => {
      res.write(chunk.data.buffer); // This writes the binary data to the response
    }, (err) => {
      if (err) {
...
app.get("/messages", async (req, res) => {
  try {
      const allMessages = await Message.find({}).select('-_id _data id body from to timestamp'); 
...
app.post('/post', async (req, res) => {
  try {
    ...
    console.log("t", newMessage._data.t);
